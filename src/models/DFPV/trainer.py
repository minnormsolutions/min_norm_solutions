...
class DFPVTrainer(object):
    ...
    def __init__(self, data_configs: Dict[str, Any], train_params: Dict[str, Any],
        ...
        if self.device_name != "cpu":
            self.treatment_1st_net.to(self.device_name)
            self.treatment_2nd_net.to(self.device_name)
            self.treatment_proxy_net.to(self.device_name)
            self.outcome_proxy_net.to(self.device_name)
            if self.backdoor_1st_net is not None:
                self.backdoor_1st_net.to(self.device_name)
                self.backdoor_2nd_net.to(self.device_name)
    ...
    def train(self, train_data: PVTrainDataSet, verbose: int = 0) -> DFPVModel:
        ...
        if self.device_name != "cpu":
            train_1st_t = train_1st_t.to_gpu(self.device_name)
            train_2nd_t = train_2nd_t.to_gpu(self.device_name)
...
def dfpv_experiments(data_config: Dict[str, Any], model_param: Dict[str, Any],
    ...
    trainer = DFPVTrainer(data_config, model_param, dump_dir, preferred_device)
