...
def generate_train_data_ate(data_config: Dict[str, Any], rand_seed: int) -> PVTrainDataSet:
    ...
    if data_name == "demand":
        ...
        return generate_train_rhc(data_config['use_all_X'])  # no random seed needed for this one
    else:
...
def generate_val_data_ate(data_config: Dict[str, Any], rand_seed: int) -> PVTrainDataSet:
    ...
    if data_name == "dsprite":
        ...
        return generate_val_rhc(data_config['use_all_X'])
    else:
        raise ValueError(f"data name {data_name} is not valid")


def generate_test_data_ate(data_config: Dict[str, Any], analysis_config: Dict[str, Any]
                            ) -> Optional[Union[PVTestDataSet, RHCTestDataSet]]:
    data_name = data_config["name"]
    if data_name == "demand":
        return generate_test_demand_pv(data_config, analysis_config)
    elif data_name == "dsprite":
        return generate_test_dsprite()
    elif data_name == "rhc":
        return generate_test_rhc(data_config['use_all_X'])
    else:
        raise ValueError(f"data name {data_name} is not valid")


def standardize(data: PVTrainDataSet) -> Tuple[PVTrainDataSet, Dict[str, StandardScaler]]:
